...
import com.vui.bluelight.music.constants.MusicConstants;
...
//	private List<LrcContent> lrcList = new ArrayList<LrcContent>(); //存放歌词列表对象
	...
	public void onStart(Intent intent, int startId) {
		...
		if (msg == MusicConstants.PlayerMsg.PLAY_MSG) {	//直接播放音乐
			play(0);
		} else if (msg == MusicConstants.PlayerMsg.PAUSE_MSG) {	//暂停
			pause();	
		} else if (msg == MusicConstants.PlayerMsg.STOP_MSG) {		//停止
			stop();
		} else if (msg == MusicConstants.PlayerMsg.CONTINUE_MSG) {	//继续播放
			resume();	
		} else if (msg == MusicConstants.PlayerMsg.PRIVIOUS_MSG) {	//上一首
			previous();
		} else if (msg == MusicConstants.PlayerMsg.NEXT_MSG) {		//下一首
			next();
		} else if (msg == MusicConstants.PlayerMsg.PROGRESS_CHANGE) {	//进度更新
			currentTime = intent.getIntExtra("progress", -1);
			play(currentTime);
		} else if (msg == MusicConstants.PlayerMsg.PLAYING_MSG) {
